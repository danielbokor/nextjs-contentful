...
import { LogoWallCollectionQuery } from "../type";
Swiper.use([Autoplay]);

export default function Clients({
  content,
}: {
  content: LogoWallCollectionQuery["logoWallCollectionCollection"]["items"];
}) {
  useEffect(() => {
          ...
          <div className="overflow-hidden">
            ...
            <div className="clients-carousel swiper-container relative before:absolute before:inset-0 before:w-32 before:z-10 before:pointer-events-none before:bg-gradient-to-r before:from-slate-900 after:absolute after:inset-0 after:left-auto after:w-32 after:z-10 after:pointer-events-none after:bg-gradient-to-l after:from-slate-900">
              <div className="swiper-wrapper !ease-linear select-none items-center">
                {/* Carousel items */}
                {content.map((item) =>
                  item.itemsCollection.items.map((logoWallItem) => (
                    <div
                      className="swiper-slide !w-auto"
                      key={logoWallItem.logo.url}
                    >
                      <Image
                        src={logoWallItem.logo.url}
                        alt={
                          logoWallItem.description || logoWallItem.logo.title
                        }
                        width={logoWallItem.logo.width}
                        height={logoWallItem.logo.height}
                      />
                    </div>
                  ))
                )}
